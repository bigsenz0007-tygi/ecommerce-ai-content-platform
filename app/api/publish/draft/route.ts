...
import { publishDemoTask } from "@/lib/demo-runtime";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    try {
      return NextResponse.json({ ...publishDemoTask(body.taskId, body.platform), fallback: true });
    } catch (err) {
      return NextResponse.json(
        { error: err instanceof Error ? err.message : e instanceof Error ? e.message : "发布预览失败" },
        { status: 400 }
      );
    }
  }
